...
export default function Shell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      {/* Sidebar */}
      <aside className={cn(
        "fixed lg:static z-50 h-screen lg:h-screen top-0 left-0 transition-all duration-300 border-r bg-card/60 backdrop-blur-xl flex flex-col",
        collapsed ? "w-20" : "w-64",
        ...
        <button onClick={() => setCollapsed(c => !c)} className="hidden lg:flex items-center justify-center text-xs text-muted-foreground hover:text-foreground py-2 border-t">
          {collapsed ? "→" : "← Collapse"}
        </button>
      </aside>
    ...
    try {
      if (user?.token) {
        const res = await fetch("http://localhost:3001/profile", {
          ...
          body: JSON.stringify({ name: form.name, phone: form.phone, gender: form.gender, address: form.address }),
        });
        if (!res.ok) {
      ...
      // Update local app state & sessionStorage
      updateProfile({ name: form.name, phone: form.phone, gender: form.gender, address: form.address });
      setEdit(false);
    } catch {
      // Auth server offline — still save locally
      updateProfile({ name: form.name, phone: form.phone, gender: form.gender, address: form.address });
      setEdit(false);
    } finally {
      setSaving(false);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] grid place-items-center p-4 bg-black/60 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <div className="grid grid-cols-2 gap-3">
          {(["name", "email", "phone", "age", "gender", "address"] as const).map(k => (
            <div key={k} className={cn(k === "address" && "col-span-2")}>
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{k}</div>
              {edit && k !== "email" && k !== "age" ? (
                <input value={form[k]} onChange={e => setForm({ ...form, [k]: e.target.value })} className="w-full mt-1 bg-card border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand/40" />
              ) : (
                <div className="text-sm font-medium mt-1 text-foreground">{form[k] || <span className="text-muted-foreground italic">Not set</span>}</div>
              )}
            </div>
          ))}
        </div>
