              ...
              {mode === "store" && (
                <motion.div key="store" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                  ...
                  <div className="grid grid-cols-3 gap-1 p-1 rounded-lg bg-muted mt-5 text-xs">
                    {(["chain", "local-login", "local-register"] as StoreSub[]).map(s => (
                      <button key={s} onClick={() => { setStoreSub(s); setError(""); }} className={cn("py-2 rounded-md font-medium transition", storeSub === s ? "bg-card shadow-sm" : "text-muted-foreground hover:text-foreground")}>
                        {s === "chain" ? "Chain (PIN)" : s === "local-login" ? "Local Login" : "Register"}
                      </button>
                    ))}
                  </div>
                  <div className="mt-5 space-y-4">
                    {storeSub === "chain" && (
                      <>
                        <Field label="Select pharmacy">
                          <select value={pharmacyId} onChange={e => setPharmacyId(e.target.value)} className="w-full bg-card border rounded-lg px-3 py-2.5 text-sm">
                            {pharmacies.filter(p => p.type === "chain").map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                          </select>
                        </Field>
                        ...
                        <div className="text-[11px] text-muted-foreground px-1">
                          Demo PINs — Apollo: <b>5678</b> · MedPlus: <b>1234</b> · Jan Aushadhi: <b>9012</b> · Others: <b>0000</b>
                        </div>
                ...
                <motion.div key="patient" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                  ...
                  <div className="mt-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <SocialBtn onClick={() => { window.location.href = "http://localhost:3001/auth/microsoft"; }} label="Microsoft" color="#00a4ef" />
                    </div>
                  </div>
                </motion.div>
              )}
...
function SocialBtn({ label, color, onClick }: { label: string; color: string; onClick: () => void }) {
  return (
